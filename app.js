/* app.js — Cornerstone Council Agent Portal */
'use strict';
...
// ── UTILS ──────────────────────────────────────────────────────
function fmtNum(n){ if(!n||isNaN(n))return'0'; n=Number(n); return n.toLocaleString('en-US',{maximumFractionDigits:0}); }
...
function tfLabel(tf){ return{today:'Today',week:'Week',month:'Month',ytd:'YTD',alltime:'All Time','1d':'Today','7d':'Last 7 Days','30d':'Last 30 Days','90d':'Last 90 Days',custom:'Custom'}[tf]||tf; }
...
// ── INTERNAL CONFIRM (replaces browser confirm) ─────────────────
...
function showConfirm(title, msg, okLabel='Confirm', danger=true){
  return new Promise(resolve=>{
    ...
    btn.className='btn '+(danger?'btn-dan':'btn-pri');
...
function _loadSession(){
  try{ const s=JSON.parse(localStorage.getItem(SESSION_KEY)); if(!s)return null; if(Date.now()-s.savedAt>INACTIVITY_MS){_clearSession();return null;} return s; }
  catch(e){return null;}
}
function _resetInactivityTimer(){
  clearTimeout(_inactivityTimer);
  const s=JSON.parse(localStorage.getItem(SESSION_KEY)||'null');
  if(s)localStorage.setItem(SESSION_KEY,JSON.stringify({...s,savedAt:Date.now()}));
  _inactivityTimer=setTimeout(()=>{doLogout();toast('Signed out due to inactivity');},INACTIVITY_MS);
}
function _startActivity(){ ['mousemove','mousedown','keydown','touchstart','scroll','click'].forEach(e=>document.addEventListener(e,_resetInactivityTimer,{passive:true})); _resetInactivityTimer(); }
function _stopActivity(){ clearTimeout(_inactivityTimer); ['mousemove','mousedown','keydown','touchstart','scroll','click'].forEach(e=>document.removeEventListener(e,_resetInactivityTimer)); }
...
function doLogin(){
  ...
  // Always sync from Firebase — works on every device, no token needed
...
function enterApp(){
  ...
  // Auto-promote to manager if they have downlines (unless already admin/owner)
  const effRole=(!['admin','owner'].includes(cur.role)&&Store.isManager(cur.agentId))?'manager':cur.role;
  const roleLabels={owner:'Owner',admin:'Admin',manager:'Manager',agent:'Agent'};
  rb.textContent=(roleLabels[effRole]||effRole).toUpperCase();
...
function switchManageTab(tab,btn){
  ...
  if(btn)btn.classList.add('active');
  document.getElementById('manage-users-div').style.display=tab==='users'?'':'none';
  document.getElementById('manage-deals-div').style.display=tab==='deals'?'':'none';
  if(tab==='users')renderManageUsers(); else if(tab==='deals')renderManageDeals();
}
...
// ── DATE FILTER ────────────────────────────────────────────────
function filterByTFOrDays(arr,tf,df,cs,ce){
  if(['1d','7d','30d','90d'].includes(tf)){
    const days={'1d':1,'7d':7,'30d':30,'90d':90}[tf];
    const cutoff=new Date(); cutoff.setDate(cutoff.getDate()-days+1); cutoff.setHours(0,0,0,0);
    return arr.filter(item=>new Date((item[df]||item.date)+'T12:00:00')>=cutoff);
  }
  return Store.filterByTF(arr,tf,df,cs,ce);
}
...
function _homeChartConfig(tf){
  ...
  if(tf==='1d'||tf==='today'){
    ...
    for(let h=0;h<24;h++){labels.push(h+':00');data.push(Store.totalAP(Store.deals.filter(d=>{const dd=new Date(d.date+'T12:00:00');return dd.toDateString()===now.toDateString()&&dd.getHours()<=h;})));}
    return{labels,data};
  }
  if(tf==='7d'||tf==='week'){
    const labels=[]; const data=[];
    for(let i=6;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);labels.push(MN[d.getMonth()]+' '+d.getDate());const ds=d.toISOString().split('T')[0];data.push(Store.totalAP(Store.deals.filter(x=>x.date===ds)));}
    return{labels,data};
  }
  if(tf==='30d'){
    const labels=[]; const data=[];
    for(let i=29;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);labels.push(i%5===0?MN[d.getMonth()]+' '+d.getDate():'');const ds=d.toISOString().split('T')[0];data.push(Store.totalAP(Store.deals.filter(x=>x.date===ds)));}
    return{labels,data};
  }
  if(tf==='90d'){
    const labels=[]; const data=[];
    for(let i=12;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i*7);labels.push(MN[d.getMonth()]+' '+d.getDate());const wk=Store.getMondayOf(d.toISOString().split('T')[0]);const wkEnd=new Date(wk+'T12:00:00');wkEnd.setDate(wkEnd.getDate()+6);data.push(Store.totalAP(Store.deals.filter(x=>{const dd=new Date(x.date+'T12:00:00');return dd>=new Date(wk+'T00:00:00')&&dd<=wkEnd;})));}
    return{labels,data};
  }
  // month / ytd / alltime — monthly buckets
...
function renderHome(){
  ...
  const filt=filterByTFOrDays(Store.deals,tf,'date');
  ...
  charts.home=new Chart(ctx,{type:'line',data:{labels,datasets:[{label:'AP',data,fill:true,borderColor:'#c9a84c',backgroundColor:'rgba(201,168,76,0.07)',tension:0.4,pointBackgroundColor:'#c9a84c',pointRadius:3,pointHoverRadius:6}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>'AP: '+fmtFull(c.raw)}}},
    scales:{x:{grid:{color:'rgba(201,168,76,0.05)'},ticks:{color:'#6a6050',font:{size:10,family:'Georgia,serif'}}},
    y:{grid:{color:'rgba(201,168,76,0.05)'},ticks:{color:'#6a6050',font:{size:10,family:'Georgia,serif'},callback:v=>fmt$(v)}}}}});
...
function renderLeaderboard(){
  ...
  charts.lb=new Chart(ctx,{type:'bar',data:{labels:lb.map(r=>r.name.split(' ')[0]),datasets:[{label:'AP',data:lb.map(r=>r.ap),backgroundColor:lb.map((_,i)=>i===0?'rgba(201,168,76,0.75)':i===1?'rgba(160,160,160,0.5)':i===2?'rgba(139,80,20,0.6)':'rgba(201,168,76,0.22)'),borderColor:lb.map((_,i)=>i===0?'#c9a84c':i===1?'#aaa':i===2?'#8a5010':'rgba(201,168,76,0.4)'),borderWidth:1,borderRadius:5}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>'AP: '+fmtFull(c.raw)}}},scales:{x:{grid:{color:'rgba(201,168,76,0.05)'},ticks:{color:'#6a6050',font:{size:10,family:'Georgia,serif'}}},y:{grid:{color:'rgba(201,168,76,0.05)'},ticks:{color:'#6a6050',font:{size:10,family:'Georgia,serif'},callback:v=>fmt$(v)}}}}});
  ...
  if(!lb.length){html+=`<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--d)">No data.</td></tr>`;}
    ...
    const rc=i===0?'r1':i===1?'r2':i===2?'r3':'rn'; const pct=maxAP>0?Math.round((r.ap/maxAP)*100):0;
    const u=Store.users.find(x=>x.agentId===r.agentId); const avImg=u?.avatar?`<img src="${u.avatar}" alt="">`:initials(r.name);
    html+=`<tr onclick="openProfile('${r.agentId}')"><td><div class="rank-badge ${rc}">${i+1}</div></td>
      <td><div class="agent-cell"><div class="agent-av-sm">${avImg}</div><div><div class="agent-nm">${r.name}</div><div class="agent-ag">${Store.agN(r.agencyId)}</div></div></div></td>
      <td><div class="ap-cell">${fmtFull(r.ap)}</div><div class="bar-wrap"><div class="bar-fill" style="width:${pct}%"></div></div></td>
      <td style="color:var(--d2)">${fmtFull(r.mp)}</td><td><span class="badge badge-g">${r.count}</span></td>
      <td style="color:var(--d2)">${r.count?fmt$(r.ap/r.count):'—'}</td></tr>`;
  });
...
function renderDashboard(){
  ...
  const ap=Store.totalAP(filt); const mp=Store.totalMP(filt);
  const totalSpend=filtSpend.reduce((s,x)=>s+(x.amount||0),0);
  ...
  document.getElementById('dash-stats').innerHTML=`
    ...
    <div class="sc"><div class="sc-label">Lead Spend — ${tfLabel(tf)}</div><div class="sc-val rd">${fmt$(totalSpend)}</div></div>`;
  ...
  if(commRate>0){
    ...
    commEl.innerHTML=`<div class="card"><div class="ct">My Earnings &amp; ROI <span class="badge badge-g" style="font-size:8px">Private</span></div>
      ...
      </div><div class="priv-box">&#128274; Visible only to you.</div></div>`;
  }else commEl.innerHTML='';
    ...
    html+=`<div class="team-card" id="tc-${u.id}">
      ...
      <div class="team-legs">
        ...
        <div class="team-member-row" onclick="openProfile('${u.agentId}')">
          ...
          <div style="flex:1"><div style="font-size:12px;font-weight:700;color:var(--t)">${u.agentName} <em style="color:var(--d);font-size:9px;font-weight:400">(team lead)</em></div>
        ...
        ${downUsers.map(du=>{
          const duD=downD.filter(d=>d.agentId===du.agentId); const avD=du.avatar?`<img src="${du.avatar}" alt="">`:initials(du.agentName);
          const canView=isAdmin()||myDownIds.includes(du.agentId)||du.agentId===cur.agentId;
          return`<div class="team-member-row" onclick="openProfile('${du.agentId}')">
            <div class="agent-av-sm">${avD}</div>
            <div style="flex:1"><div style="font-size:12px;font-weight:700;color:var(--t)">${du.agentName}</div>
            <div style="font-size:10px;color:var(--d2)">${fmt$(Store.totalAP(duD))} AP · ${duD.length} deals</div></div>
            ${canView?`<button class="btn btn-ghost btn-xs" onclick="openAgentRpt('${du.agentId}',event)">Report</button>`:''}
          </div>`;
        }).join('')}
...
function openAgentRpt(agentId,e){
  ...
  const u=Store.users.find(x=>x.agentId===agentId); if(!u)return;
  ...
  </div>${myD.slice(0,20).map(d=>`<div class="deal-row" style="cursor:default"><div class="deal-top"><div><div class="fw7 text-g">${fmtFull(d.ap)} AP</div><div class="deal-meta">${fmtDate(d.date)} · MP ${fmt$(d.mp)} ${d.policyType?'· '+d.policyType:''} ${d.carrier?'· '+d.carrier:''}</div></div></div></div>`).join('')}`;
...
function renderConsistency(){
  ...
  au.forEach(u=>{
    ...
    const cells=[]; for(let i=363;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const ds=d.toISOString().split('T')[0];const cnt=dayMap[ds]||0;const cls=cnt===0?'hm-0':cnt===1?'hm-1':cnt<=3?'hm-2':'hm-3';cells.push(`<div style="width:10px;height:10px;border-radius:2px" class="hm-cell ${cls}" title="${ds}: ${cnt} deal${cnt!==1?'s':''}"></div>`);}
    ...
    html+=`<div class="card">
      <div class="flsb" style="margin-bottom:10px">
        <div class="fl" style="gap:10px">
          <div class="agent-av-sm" style="cursor:pointer" onclick="openProfile('${u.agentId}')">${avImg}</div>
          <div><div class="fw7" style="cursor:pointer" onclick="openProfile('${u.agentId}')">${u.agentName}</div><div class="text-xs text-d">${Store.agN(Store.getMember(u.agentId)?.agency)}</div></div>
        </div>
        <div><div style="font-size:10px;color:var(--d2);margin-bottom:4px">Current streak: <strong style="color:var(--g)">${dayStreak} day${dayStreak!==1?'s':''}</strong> · ${activeDays}/${totalDays} days active (${pct}%)</div>
        <div>${earned.length?earned.map(m=>`<span class="streak-pill" style="color:var(--g)">&#128293; ${m}d</span>`).join(''):'<span class="streak-pill" style="color:var(--d)">No streak yet</span>'}</div></div>
      </div>
      <div style="font-size:9px;color:var(--d);margin-bottom:4px">Last 364 days — darker = more deals</div>
...
function renderIncentives(){
  ...
  if(!sorted.length){document.getElementById('incentives-body').innerHTML=`<div class="empty" style="grid-column:1/-1">No incentives yet.${isOwner()?' Click "+ Add Incentive" to create one.':''}</div>`;return;}
  ...
  const tfMap={weekly:'Weekly',monthly:'Monthly',quarterly:'Quarterly',semiannual:'Semi-Annual',annual:'Annual'};
  let html='';
  sorted.forEach(inc=>{
    const isActive=(!inc.endDate||inc.endDate>=now)&&(!inc.startDate||inc.startDate<=now);
    const goal=parseFloat(inc.goal)||0; const scope=inc.scope||'agency';
    const tfD=Store.filterByTF(Store.deals,'custom','date',inc.startDate||null,inc.endDate||null);
    let myAP=0,agencyAP=0,pct=0;
    if(scope==='agency'){agencyAP=Store.totalAP(tfD);pct=goal>0?Math.min(100,Math.round((agencyAP/goal)*100)):0;}
    else if(scope==='personal'){myAP=cur.agentId?Store.totalAP(tfD.filter(d=>d.agentId===cur.agentId)):0;pct=goal>0?Math.min(100,Math.round((myAP/goal)*100)):0;}
    else if(scope==='team'){const tIds=cur.agentId?[cur.agentId,...Store.getDownlineIds(cur.agentId)]:[];myAP=Store.totalAP(tfD.filter(d=>tIds.includes(d.agentId)));pct=goal>0?Math.min(100,Math.round((myAP/goal)*100)):0;}
    const progressAP=scope==='agency'?agencyAP:myAP;
    const canRedeem=goal>0&&progressAP>=goal&&cur.agentId;
    ...
    html+=`<div class="inc-card ${isActive?'inc-active':''}">
      ...
      ${goal?`<div style="font-size:10px;color:var(--d2)">Goal: ${fmtFull(goal)} AP</div>
        <div class="inc-prog-wrap"><div class="inc-prog-fill" style="width:${pct}%"></div></div>
        <div style="font-size:10px;color:var(--d2)">${fmtFull(progressAP)} / ${fmtFull(goal)} — ${pct}%</div>`:''}
      ${inc.desc?`<div style="font-size:10px;color:var(--d);line-height:1.5">${inc.desc}</div>`:''}
      ${canRedeem&&!alreadyEarned?`<button class="redeem-btn" onclick="redeemInc('${inc.id}')">&#9650; Redeem Badge</button>`:''}
      ${alreadyEarned?`<div style="font-size:10px;color:#81c784;margin-top:4px">&#10003; Badge earned</div>`:''}
      ${isOwner()?`<div class="fl" style="margin-top:8px;gap:6px"><button class="btn btn-ghost btn-xs" onclick="editInc('${inc.id}')">Edit</button><button class="btn btn-dan btn-xs" onclick="delInc('${inc.id}')">Remove</button></div>`:''}
    </div>`;
  });
...
async function redeemInc(incId){
  ...
  if(u.badges.some(b=>b.incId===incId)){toast('Already earned this badge');return;}
  u.badges.push({id:Store.uid(),incId,emoji:inc.emoji||'🏆',name:inc.badgeTitle||inc.title,earnedAt:new Date().toISOString()});
  cur.badges=u.badges;
...
function editInc(id){
  ...
  document.getElementById('inc-title').value=inc.title||''; document.getElementById('inc-tf').value=inc.timeframe||'monthly';
  document.getElementById('inc-scope').value=inc.scope||'agency'; document.getElementById('inc-start').value=inc.startDate||'';
  document.getElementById('inc-end').value=inc.endDate||''; document.getElementById('inc-goal').value=inc.goal||'';
...
async function saveInc(){
  ...
  const obj={id:id||Store.uid(),title:document.getElementById('inc-title').value.trim(),timeframe:document.getElementById('inc-tf').value,scope:document.getElementById('inc-scope').value,startDate:document.getElementById('inc-start').value,endDate:document.getElementById('inc-end').value,goal:parseFloat(document.getElementById('inc-goal').value)||0,reward:document.getElementById('inc-reward').value.trim(),desc:document.getElementById('inc-desc').value.trim(),emoji:document.getElementById('inc-emoji').value.trim(),badgeTitle:document.getElementById('inc-badge-title').value.trim(),createdAt:new Date().toISOString()};
...
async function delInc(id){
  const ok=await showConfirm('Remove Incentive','Remove this incentive? This cannot be undone.','Remove');
  if(!ok)return;
  const idx=Store.incentives.findIndex(x=>x.id===id); if(idx>-1)Store.incentives.splice(idx,1);
  await Store.saveAll(); renderIncentives(); toast('Removed');
}

// ── MANAGE ─────────────────────────────────────────────────────
function renderManage(){
  if(!isAdmin())return;
  document.getElementById('manage-users-div').style.display=manageTab==='users'?'':'none';
  document.getElementById('manage-deals-div').style.display=manageTab==='deals'?'':'none';
  if(manageTab==='users')renderManageUsers(); else if(manageTab==='deals')renderManageDeals();
}
function renderManageUsers(){
  Store.loadLocal();
  const registered=Store.users.filter(u=>u.agentId).map(u=>u.agentId);
  let html=`<div class="ct">Portal Accounts</div>`;
  Store.users.filter(u=>u.active!==false).forEach(u=>{
    const m=Store.getMember(u.agentId); const rc={agent:'badge-grn',admin:'badge-g',owner:'rb-owner'}[u.role]||'badge-grey';
    const av=u.avatar?`<img src="${u.avatar}" alt="">`:initials(u.agentName||u.username);
    const isProtected=['u_jc','u_ty'].includes(u.id);
    html+=`<div class="user-row">
      <div class="user-row-left"><div class="agent-av-sm">${av}</div>
        <div><div class="fw7">${u.agentName||u.username} <span class="badge ${rc}">${u.role}</span></div>
        <div class="text-xs text-d" style="margin-top:2px">@${u.username} ${m?'· '+Store.agN(m.agency):''} ${u.commission?'· '+u.commission+'%':''}</div></div>
      </div>
      <div class="user-row-right">
        <button class="btn btn-ghost btn-xs" onclick="openProfile('${u.agentId||''}',null,'${u.id}')">Profile</button>
  ...
  const unregistered=Store.crmMembers.filter(m=>!registered.includes(m.id));
  if(unregistered.length){
    html+=`<div class="ct" style="margin-top:16px">Hierarchy — No Portal Account</div>`;
    unregistered.forEach(m=>{
      html+=`<div class="user-row">
        <div class="user-row-left"><div class="agent-av-sm" style="background:var(--p3)">${initials(m.name)}</div>
        <div><div class="fw7" style="color:var(--d2)">${m.name}</div><div class="text-xs text-d">${Store.agN(m.agency)} · No account</div></div></div>
...
function renderReports(){
  ...
  document.getElementById('rpt-preview').innerHTML=`<div class="card"><div class="ct">Preview — ${tfLabel(tf)}</div>
    ...
    <div class="tbl-wrap"><table class="tbl"><thead><tr><th>Rank</th><th>Agent</th><th>AP</th><th>MP</th><th>Deals</th><th>Avg AP</th></tr></thead><tbody>
      ${lb.map((r,i)=>`<tr><td><div class="rank-badge ${i===0?'r1':i===1?'r2':i===2?'r3':'rn'}">${i+1}</div></td><td><div class="agent-nm">${r.name}</div><div class="agent-ag">${Store.agN(r.agencyId)}</div></td><td class="ap-cell">${fmt$(r.ap)}</td><td style="color:var(--d2)">${fmt$(r.mp)}</td><td>${r.count}</td><td style="color:var(--d2)">${r.count?fmt$(r.ap/r.count):'—'}</td></tr>`).join('')}
    </tbody></table></div></div>`;
}

// Sign performance — PIE CHART
function renderAstroSection(){
  ...
  const bySign={};
  Store.getAgentUsers().forEach(u=>{
    const m=Store.getMember(u.agentId); if(!m?.dob)return;
    const sign=m.zodiac||'?'; const myAP=Store.totalAP(filt.filter(d=>d.agentId===u.agentId));
    if(!bySign[sign])bySign[sign]={agents:[],total:0}; bySign[sign].agents.push(u.agentName); bySign[sign].total+=myAP;
  });
  ...
  const ctx=document.getElementById('chart-astro-pie')?.getContext('2d');
  if(ctx){
    if(charts.astro)charts.astro.destroy();
    charts.astro=new Chart(ctx,{type:'pie',
      data:{labels:signData.map(([sign])=>`${ZE[sign]||''} ${cap(sign)}`),datasets:[{data:signData.map(([,d])=>d.total),backgroundColor:pieColors.slice(0,signData.length),borderColor:'#000000',borderWidth:2}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{color:'#9a8f7a',font:{size:11,family:'Georgia,serif'},padding:12}},tooltip:{callbacks:{label:c=>`${c.label}: ${fmtFull(c.raw)}`}}}}});
  }
  const rows=signData.map(([sign,d])=>`<tr><td><span class="sign-chip">${ZE[sign]||''} ${cap(sign)}</span></td><td class="ap-cell">${fmt$(d.total)}</td><td style="font-size:10px;color:var(--d2)">${d.agents.join(', ')}</td></tr>`).join('');
  document.getElementById('astro-body').innerHTML=`<div class="chart-wrap pie"><canvas id="chart-astro-pie"></canvas></div>
    <div class="tbl-wrap" style="margin-top:14px"><table class="tbl"><thead><tr><th>Sign</th><th>AP</th><th>Agents</th></tr></thead><tbody>${rows||'<tr><td colspan="3" class="empty">No DOB data</td></tr>'}</tbody></table></div>
    <div class="info-box mt8">&#9670; Owner-only. Not visible to agents or admins.</div>`;
  // Re-init the canvas and chart since innerHTML replaced it
  setTimeout(()=>{
    const ctx2=document.getElementById('chart-astro-pie')?.getContext('2d');
    if(!ctx2)return;
    if(charts.astro)charts.astro.destroy();
    charts.astro=new Chart(ctx2,{type:'pie',
      data:{labels:signData.map(([sign])=>`${ZE[sign]||''} ${cap(sign)}`),datasets:[{data:signData.map(([,d])=>d.total),backgroundColor:pieColors.slice(0,signData.length),borderColor:'#000000',borderWidth:2}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{color:'#9a8f7a',font:{size:11,family:'Georgia,serif'},padding:12}},tooltip:{callbacks:{label:c=>`${c.label}: ${fmtFull(c.raw)}`}}}}});
  },100);
}
...
function generateCurrentMonthReport(){_buildMonthlyReport(new Date());toast('Monthly report saved');setTimeout(renderReports,400);}
function _buildMonthlyReport(forDate){
  ...
  const report={monthKey:key,month:MNF[mo]+' '+yr,generatedAt:new Date().toISOString(),totalAP:Store.totalAP(monthDeals),totalDeals:monthDeals.length,activeAgents:au.filter(u=>monthDeals.some(d=>d.agentId===u.agentId)).length,totalAgents:au.length,newSignups:Store.users.filter(u=>u.createdAt&&u.createdAt.startsWith(key)).length,
    ...
    return{id:u.agentId,name:u.agentName,agency:Store.agN(Store.getMember(u.agentId)?.agency),ap:Store.totalAP(myD),deals:myD.length,activeDays:Object.keys(dayMap).length,streak};}).sort((a,b)=>b.ap-a.ap)};
...
function _renderMonthlyReportsList(){
  ...
  if(!reports.length){document.getElementById('monthly-reports-list').innerHTML='<div class="empty">No reports yet. Click "Save This Month".</div>';return;}
  document.getElementById('monthly-reports-list').innerHTML=reports.map(r=>`<div class="report-card">
    <div><div class="report-month">${r.month}</div><div style="font-size:11px;color:var(--d2);margin-top:2px">${fmtFull(r.totalAP)} AP · ${r.totalDeals} deals · ${r.activeAgents}/${r.totalAgents} agents ${r.newSignups?'· '+r.newSignups+' new':''}</div></div>
    <button class="btn btn-ghost btn-sm" onclick="downloadMonthlyReport('${r.monthKey}')">&#8681; Download</button>
  </div>`).join('');
}
function downloadMonthlyReport(monthKey){
  ...
  const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cornerstone ${r.month}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Georgia,serif;background:#fff;color:#111;padding:32px}h1{font-size:24px;color:#c9a84c;margin-bottom:4px}h2{font-size:13px;color:#8a6e30;margin-bottom:20px;font-weight:400;letter-spacing:1px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px}.box{border:1px solid #e0d0a0;border-radius:8px;padding:12px;text-align:center}.val{font-size:26px;font-weight:700;color:#c9a84c}.lbl{font-size:9px;color:#888;text-transform:uppercase;letter-spacing:1px;margin-top:4px}table{width:100%;border-collapse:collapse;font-size:12px}th{padding:9px 12px;text-align:left;font-size:9px;color:#8a6e30;text-transform:uppercase;letter-spacing:.8px;border-bottom:1px solid #e0d0a0}td{padding:9px 12px;border-bottom:1px solid #f5f0e8;color:#444}tr:nth-child(even)td{background:#fdfaf4}.gold{color:#c9a84c;font-weight:700}.footer{margin-top:28px;font-size:10px;color:#bbb;border-top:1px solid #e0d0a0;padding-top:12px;text-align:center}</style></head><body>
  ...
  <table><thead><tr><th>Rank</th><th>Agent</th><th>Agency</th><th>AP</th><th>Deals</th><th>Active Days</th><th>Streak</th><th>Avg AP</th></tr></thead><tbody>
  ${r.agentStats.map((a,i)=>`<tr><td>${i+1}</td><td>${a.name}</td><td>${a.agency}</td><td class="gold">$${fmtNum(a.ap)}</td><td>${a.deals}</td><td>${a.activeDays}</td><td>${a.streak}d</td><td>${a.deals?'$'+fmtNum(Math.round(a.ap/a.deals)):'—'}</td></tr>`).join('')}
  </tbody></table><div class="footer">Cornerstone Council · ${r.month} · Generated ${new Date(r.generatedAt).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div></body></html>`;
  ...
  toast(`${r.month} downloaded — open in browser &amp; Print → Save as PDF`);
}
function setTF(view,tf,btn){
  TF[view]=tf;
  const viewIds={home:'home',lb:'leaderboard',dash:'dashboard',teams:'teams',cons:'consistency',rpt:'reports'};
...
let _profileAgentId=null;let _profileUserId=null;
function openProfile(agentId,e,fallbackUserId){
  ...
  _renderProfileModal(u);
  document.getElementById('profile-modal-ov').classList.add('open');
}
function openOwnProfile(){if(cur)openProfile(cur.agentId,null,cur.id);}
function closeProfile(){document.getElementById('profile-modal-ov').classList.remove('open');}

function _renderProfileModal(u){
  const m=u.agentId?Store.getMember(u.agentId):null;
  const isOwnProfile=u.id===cur?.id;
  const canEditJoin=isOwner();
  ...
  // Socials
  const socialLinks=Object.entries(u.links||{}).filter(([,v])=>v).map(([k,v])=>{
    let href=v; if(!/^https?:\/\//i.test(href))href='https://'+href;
    return`<a href="${href}" target="_blank" rel="noopener" class="social-link">${cap(k)}</a>`;
  }).join('');
  document.getElementById('profile-modal-title').textContent=u.agentName||u.username;
  // Build tabs: Deals, Badges, Edit Profile (own only)
  const tabs=[
    {id:'deals',label:'Deals'},
    {id:'badges',label:`Badges ${u.badges?.length?`(${u.badges.length})`:''}`},
  ...
  html+=`<div class="profile-tabs">${tabs.map(t=>`<button class="tab${t.id==='deals'?' active':''}" onclick="_profileTab('${t.id}',this)">${t.label}</button>`).join('')}</div>`;
  // Deals tab
  html+=`<div id="pt-deals">${myDeals.length?myDeals.map(d=>`<div class="deal-row clickable" onclick="this.classList.toggle('expanded')"><div class="deal-top"><div><div class="fw7 text-g">${fmtFull(d.ap)} AP</div><div class="deal-meta">${fmtDate(d.date)} · MP ${fmt$(d.mp)} ${d.policyType?'· '+d.policyType:''} ${d.carrier?'· '+d.carrier:''}</div></div></div><div class="deal-notes-body">${d.notes||'<em style="color:var(--d)">No notes.</em>'}</div></div>`).join(''):'<div class="empty">No deals posted.</div>'}</div>`;
  // Badges tab
  const badges=u.badges||[];
  html+=`<div id="pt-badges" style="display:none">
    ...
    ${badges.length?`<div class="badge-grid">${badges.map(b=>`<div class="earned-badge">
      <div class="badge-emoji">${b.emoji||'🏅'}</div>
      <div class="badge-name">${b.name}</div>
      <div class="badge-from">${fmtDate(b.earnedAt?.split('T')[0])}</div>
      ${b.id===u.wornBadgeId?'<div class="badge-worn-dot" title="Currently worn"></div>':''}
    ...
    </div>`).join('')}</div>`:'<div class="empty">No badges earned yet.</div>'}
  </div>`;
  // Edit tab
  if(isOwnProfile){
    html+=`<div id="pt-edit" style="display:none">
      <div class="fg"><label>Bio / Description</label><textarea id="prof-bio" rows="4" placeholder="Write a short bio...">${u.bio||''}</textarea></div>
      <div class="fr2">
        <div class="fg"><label>Instagram</label><input type="url" id="prof-ig" value="${u.links?.instagram||''}" placeholder="https://instagram.com/..."></div>
        <div class="fg"><label>LinkedIn</label><input type="url" id="prof-li" value="${u.links?.linkedin||''}" placeholder="https://linkedin.com/in/..."></div>
      </div>
      <div class="fr2">
        <div class="fg"><label>Facebook</label><input type="url" id="prof-fb" value="${u.links?.facebook||''}" placeholder="https://facebook.com/..."></div>
        <div class="fg"><label>Website</label><input type="url" id="prof-web" value="${u.links?.website||''}" placeholder="https://..."></div>
      </div>
...
function _profileTab(tab,btn){
  document.querySelectorAll('#profile-modal-body .tab').forEach(b=>b.classList.remove('active'));
  if(btn)btn.classList.add('active');
  ['deals','badges','edit'].forEach(t=>{const el=document.getElementById('pt-'+t);if(el)el.style.display=t===tab?'':'none';});
}
...
function handleAvatarUpload(e){
  ...
  reader.onload=async ev=>{
    const u=Store.users.find(x=>x.id===_profileUserId);if(!u)return;
    u.avatar=ev.target.result;
    if(cur.id===_profileUserId){_setHdrAvatar(cur);cur.avatar=u.avatar;}
    await Store.saveAll();toast('Photo updated');_renderProfileModal(u);
  };
...
async function saveProfileEdits(uid){
  ...
  u.links={instagram:document.getElementById('prof-ig')?.value||'',linkedin:document.getElementById('prof-li')?.value||'',facebook:document.getElementById('prof-fb')?.value||'',website:document.getElementById('prof-web')?.value||''};
  await Store.saveAll();toast('Profile saved');_renderProfileModal(u);
}
...
async function toggleWearBadge(userId,badgeId){
  const u=Store.users.find(x=>x.id===userId);if(!u)return;
  u.wornBadgeId=u.wornBadgeId===badgeId?null:badgeId;
  if(cur.id===userId){cur.wornBadgeId=u.wornBadgeId;_updateWornBadge();}
  await Store.saveAll();_renderProfileModal(u);toast(u.wornBadgeId?'Badge worn':'Badge removed from display');
}

function openGiveBadge(agentId){
  document.getElementById('gb-agent-id').value=agentId;
  document.getElementById('gb-emoji').value=''; document.getElementById('gb-name').value=''; document.getElementById('gb-note').value='';
  openMod('modal-give-badge');
}
async function giveBadge(){
  const agentId=document.getElementById('gb-agent-id').value;
  const emoji=document.getElementById('gb-emoji').value.trim(); const name=document.getElementById('gb-name').value.trim();
  if(!name){toast('Badge name required');return;}
  const u=Store.users.find(x=>x.agentId===agentId);if(!u)return;
  if(!u.badges)u.badges=[];
  u.badges.push({id:Store.uid(),emoji,name,note:document.getElementById('gb-note').value.trim(),earnedAt:new Date().toISOString(),manual:true});
  await Store.saveAll();closeMod('modal-give-badge');toast(`${emoji} Badge given to ${u.agentName}`);
  const pU=Store.users.find(x=>x.agentId===agentId);if(pU)_renderProfileModal(pU);
}
async function removeBadge(userId,badgeId){
  const ok=await showConfirm('Remove Badge','Remove this badge from the agent?','Remove');if(!ok)return;
  const u=Store.users.find(x=>x.id===userId);if(!u)return;
  u.badges=(u.badges||[]).filter(b=>b.id!==badgeId);
  if(u.wornBadgeId===badgeId)u.wornBadgeId=null;
  if(cur.id===userId){cur.badges=u.badges;cur.wornBadgeId=u.wornBadgeId;_updateWornBadge();}
  await Store.saveAll();_renderProfileModal(u);toast('Badge removed');
}
    ...
    <div class="deal-top">
      <div style="flex:1;min-width:0">
        <div class="fl" style="gap:7px;flex-wrap:wrap">
          ...
          ${d.carrier?`<span class="badge badge-grey">${d.carrier}</span>`:''}
        </div>
        <div class="deal-meta">${fmtDate(d.date)} · MP ${fmt$(d.mp)}${d.notes?' · <span style="color:var(--g3)">&#8659; notes</span>':''}</div>
      </div>
...
function openPostDeal(){
  ...
  document.getElementById('pd-carrier').value=''; document.getElementById('pd-ptype').value=''; document.getElementById('pd-notes').value='';
  document.getElementById('pd-date').value=new Date().toISOString().split('T')[0]; document.getElementById('deal-err').style.display='none'; openMod('modal-deal');
}
function openEditDeal(id,e){
  if(e)e.stopPropagation(); const d=Store.deals.find(x=>x.id===id);if(!d)return;
...
async function submitDeal(){
  ...
  if(id){const d=Store.deals.find(x=>x.id===id);if(d){d.date=date;d.mp=parseFloat(mp.toFixed(2));d.ap=parseFloat((mp*12).toFixed(2));d.carrier=document.getElementById('pd-carrier').value.trim();d.policyType=document.getElementById('pd-ptype').value;d.notes=document.getElementById('pd-notes').value.trim();d.editedAt=new Date().toISOString();}}
  else{Store.deals.push({id:Store.uid(),agentId:cur.agentId,agentName:cur.agentName,agencyId:cur.agencyId,date,mp:parseFloat(mp.toFixed(2)),ap:parseFloat((mp*12).toFixed(2)),carrier:document.getElementById('pd-carrier').value.trim(),policyType:document.getElementById('pd-ptype').value,notes:document.getElementById('pd-notes').value.trim(),createdAt:new Date().toISOString(),editedAt:null});}
  await Store.saveAll(); closeMod('modal-deal'); toast((id?'Updated':'Submitted')+' — '+fmtFull(mp*12)+' AP'); refresh();
}
async function delDeal(id,e){
  if(e)e.stopPropagation(); const d=Store.deals.find(x=>x.id===id);if(!d)return;
  if(!isOwner()&&!isAdmin()&&d.agentId!==cur.agentId){toast('You can only remove your own deals.');return;}
  const ok=await showConfirm('Remove Deal','Remove this deal? Press Ctrl+Z after to undo.','Remove');if(!ok)return;
  const idx=Store.deals.findIndex(x=>x.id===id);if(idx>-1)Store.deals.splice(idx,1);
  await Store.saveAll(); toast('Deal removed — Ctrl+Z to undo'); refresh();
}
...
async function submitSpend(){
  const amount=parseFloat(document.getElementById('sp-amount').value);const weekOf=document.getElementById('sp-week').value;
  if(!amount||amount<=0){toast('Enter a valid amount');return;} if(!cur.agentId){toast('Not linked to agent account');return;}
  Store.spend.push({id:Store.uid(),agentId:cur.agentId,agentName:cur.agentName,weekOf,amount:parseFloat(amount.toFixed(2)),notes:document.getElementById('sp-notes').value.trim(),createdAt:new Date().toISOString()});
  await Store.saveAll();closeMod('modal-spend');toast('Lead spend recorded — '+fmt$(amount));refresh();
}
async function delSpend(id,e){
  if(e)e.stopPropagation();
  const ok=await showConfirm('Remove Lead Spend','Remove this entry?','Remove');if(!ok)return;
  const idx=Store.spend.findIndex(x=>x.id===id);if(idx>-1)Store.spend.splice(idx,1);
  await Store.saveAll();toast('Removed');refresh();
}
...
async function saveUser(){
  const uid=document.getElementById('u-id').value;const username=document.getElementById('u-username').value.trim();
  const pwd=document.getElementById('u-password').value.trim();const comm=document.getElementById('u-comm').value.trim();
  const role=document.getElementById('u-role').value||'agent';const joinDate=document.getElementById('u-join-date').value;
...
async function deactivateUser(uid){
  ...
  const ok=await showConfirm('Deactivate Account','This agent will no longer be able to log in. Their deals and data are preserved.','Deactivate');if(!ok)return;
  const u=Store.users.find(x=>x.id===uid);if(!u)return;
  u.active=false;
  // Do NOT delete their deals or data — just mark inactive
  await Store.saveAll();renderManageUsers();toast('Account deactivated — data preserved');
}

// ── FIREBASE / DB ──────────────────────────────────────────────
function openGHModal(){ openMod('modal-gh'); GH.updateStatus(true); }
// Legacy stubs — no longer needed with Firebase but kept so nothing breaks
async function testGH(){ toast('Firebase is always connected — no token needed'); }
async function saveGHConfig(){ closeMod('modal-gh'); toast('Firebase configured automatically'); }
...
function dlReport(type){Store.loadLocal();const tf=TF.rpt;const cs=document.getElementById('rpt-cs')?.value||null;const ce=document.getElementById('rpt-ce')?.value||null;let csv='',filename='';
  if(type==='lb'){const lb=Store.buildLB(tf,cs,ce);csv='Rank,Agent,Agency,AP,MP,Deals,Avg AP\n';lb.forEach((r,i)=>{csv+=`${i+1},"${r.name}","${Store.agN(r.agencyId)}",${r.ap},${r.mp},${r.count},${r.count?Math.round(r.ap/r.count):0}\n`;});filename=`cornerstone_lb_${tf}.csv`;}
  else if(type==='deals'){const d=Store.filterByTF(Store.deals,tf,'date',cs,ce).sort((a,b)=>new Date(b.date)-new Date(a.date));csv='Date,Agent,Policy Type,Carrier,MP,AP\n';d.forEach(x=>{const u=Store.users.find(y=>y.agentId===x.agentId);csv+=`"${x.date}","${u?.agentName||x.agentId}","${x.policyType||''}","${x.carrier||''}",${x.mp},${x.ap}\n`;});filename=`cornerstone_deals_${tf}.csv`;}
